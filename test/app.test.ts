...
let sf: typeof Framework;
let dai: Contract;
let daix: WrapperSuperToken;
let superSigner: SuperToken;
...
before(async function () {
  ...
  let App = await ethers.getContractFactory('TradeableFlow', accounts[0]);

  TradeableCashflow = await App.deploy('TradeableFlow', 'TCF', sf.settings.config.hostAddress);
});
...
describe('Changing owner', async function () {
  it.skip('Case #5 - When the owner changes, the flow changes', async () => {
    const initialOwnerFlowRate = await sf.cfaV1.getNetFlow({
      superToken: daix.address,
      account: accounts[1].address,
      providerOrSigner: superSigner,
